...
token next_token(YYCTYPE * str){
    ...
    YYCTYPE * YYMARKER = 0;
        ...
        /*!re2c
                        ...
                        ([0-9]+("\.")?[0-9]+) { new_token.type = WEIGHT; new_token.text = strndup(start, YYCURSOR - start); new_token.next = YYCURSOR; return new_token;}
                        ([a-zA-Z_0-9_-]+) { new_token.type = ID; new_token.text = strndup(start, YYCURSOR - start); new_token.next = YYCURSOR; return new_token;}
